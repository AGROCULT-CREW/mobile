...
public static class NativeCamera
{
	...
	public static Permission RequestPermission(string line = "NativeCameraPermission")
	{
#if !UNITY_EDITOR && UNITY_ANDROID
		object threadLock = new object();
		lock( threadLock )
		{
			NCPermissionCallbackAndroid nativeCallback = new NCPermissionCallbackAndroid( threadLock, line);

			AJC.CallStatic( "RequestPermission", Context, nativeCallback, PlayerPrefs.GetInt( line, (int) Permission.ShouldAsk ) );

			if( nativeCallback.Result == -1 )
				System.Threading.Monitor.Wait( threadLock );

			if( (Permission) nativeCallback.Result != Permission.ShouldAsk && PlayerPrefs.GetInt( line, -1 ) != nativeCallback.Result )
			{
				PlayerPrefs.SetInt( line, nativeCallback.Result );
				PlayerPrefs.Save();
			}
